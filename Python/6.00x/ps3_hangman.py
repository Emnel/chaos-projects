...
WORDLIST_FILENAME = "E:/Projects/Python/6.00x/words.txt"
...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    print 'I am thinking of the word that is %d letters long.' % len(secretWord)
    while mistakesMade < 8 and not isWordGuessed(secretWord, lettersGuessed):
        ...
        aGuess = raw_input('Please guess a letter: ')
        if aGuess.lower() in lettersGuessed:
            print 'Oops! You\'ve already guessed that letter: %s' % (getGuessedWord(secretWord, lettersGuessed))
        else:
            lettersGuessed.append(aGuess.lower())
            if aGuess.lower() in secretWord:
