...
def loadWords():
    ...
    # inFile: file
    inFile = open("E:/Projects/chaos-projects/Python/6.00x/Word Game/ProblemSet4/words.txt", 'r', 0)
...
def updateHand(hand, word):
    ...
    for x in word:
        updatedHand[x] = updatedHand.get(x,0) - 1
    return updatedHand
...
def isValidWord(word, hand, wordList):
    ...
    theHand = hand.copy()
    for x in word:
        theHand[x] = theHand.get(x,0) - 1
    for x in theHand.values():
        if x < 0:
            return False
    return True
#
